...
import { PaginationInfo } from "../../../types/eventos"

interface PaginacionSuperiorEventosProps {
...
function getNombreFiltro(filtro: string) {
  switch (filtro) {
    case 'todos': return 'Todos los eventos'
    case 'En proceso': return 'Eventos en proceso'
    case 'Aceptado': return 'Eventos aceptados'
    case 'Rechazado': return 'Eventos rechazados'
...
export default function PaginacionSuperiorEventos({
  pagination,
  currentPage,
  onPageChange,
  loading = false,
  filtro
}: PaginacionSuperiorEventosProps) {
  if (!pagination) return null

  const numerosPagina = generarNumerosPagina(pagination)
  const itemsPerPage = 10 // Ajusta según tu API
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
        <div>
          <p className="text-sm text-gray-600">
            Mostrando <span className="font-semibold">{(currentPage - 1) * itemsPerPage + 1}-{Math.min(currentPage * itemsPerPage, pagination.totalCount)}</span> de <span className="font-semibold">{pagination.totalCount}</span> {getNombreFiltro(filtro).toLowerCase()}
          </p>
        </div>
