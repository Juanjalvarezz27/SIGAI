...
export default function MiniModalBusqueda({
  ...
  const cargarModelosPorMarca = useCallback(async () => {
    if (!marcaId) return
  ...
  }, [isOpen, valorActual, buscarItemsDebounced, tipo, marcaId, cargarModelosPorMarca])
  ...
  const handleCrearNuevo = () => {
    // Siempre mostrar el input al hacer click en el botón verde
    setMostrarInputNuevo(true)
    setNombreNuevo(busqueda) // Pre-llenar con la búsqueda actual si existe
  }

  const handleGuardarNuevo = () => {
    if (nombreNuevo.trim()) {
      onSeleccionar("otro", nombreNuevo.trim())
      onClose()
    }
  }
  ...
  const handleKeyPress = (e: React.KeyboardEvent) => {
    if (e.key === 'Escape') {
      if (mostrarInputNuevo) {
        handleCancelarCreacion()
      } else {
        onClose()
      }
    }
    if (e.key === 'Enter' && mostrarInputNuevo && nombreNuevo.trim()) {
      handleGuardarNuevo()
    }
  }
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {/* Barra de búsqueda - Solo mostrar si no está en modo creación */}
          {!mostrarInputNuevo && (
            <div className="relative mb-4">
              ...
              <input
                ...
                placeholder={tipo === "marca" ? "Buscar marcas..." : "Buscar modelos..."}
                autoFocus
              />
            </div>
          )}
          ...
          {tipo === "modelo" && marcaId && !mostrarInputNuevo && (
          ...
          {mostrarInputNuevo && (
            <div className="mb-4 p-4 bg-green-50 border border-green-200 rounded-lg">
              <h4 className="font-medium text-green-800 mb-3">
                Crear nuevo {tipo === "marca" ? "marca" : "modelo"}
              </h4>
              <div className="space-y-3">
                <input
                  ...
                  placeholder={`Escribe el nombre del ${tipo === "marca" ? "marca" : "modelo"}...`}
                  autoFocus
                />
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 px-4 py-2 cursor-pointer bg-green-600 text-white rounded-md hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    Guardar {tipo === "marca" ? "Marca" : "Modelo"}
                  </button>
                  <button
          ...
          {/* Botón para crear nuevo - Solo mostrar si no está en modo creación */}
          {!mostrarInputNuevo && (
            <button
              onClick={handleCrearNuevo}
              className="w-full p-4 bg-green-50 border border-green-200 rounded-lg hover:bg-green-100 transition-colors mb-4 cursor-pointer"
            >
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  <span className="font-medium text-green-800 block">
                    {busqueda.trim()
                      ? `Crear "${busqueda}" como nuevo ${tipo}`
                      : `Crear nueva ${tipo}`
                    }
                  </span>
                  <p className="text-sm text-green-600 mt-1">
                    {busqueda.trim()
                      ? `Haz click para confirmar y crear "${busqueda}"`
                      : `Haz click para escribir el nombre del ${tipo}`
                    }
                  </p>
                </div>
              </div>
          ...
          {tipo === "marca" && busqueda && busqueda.length < 2 && !mostrarInputNuevo && (
          ...
          {/* Loader */}
          {cargando && !mostrarInputNuevo && (
            <div className="flex justify-center py-4">
              <div className="w-6 h-6 border-2 border-[#001F3F] border-t-transparent rounded-full animate-spin"></div>
            </div>
          )}

          {/* Lista de resultados - SOLO mostrar si hay items y no está en modo creación */}
          {!cargando && items.length > 0 && !mostrarInputNuevo && (
          ...
          {tipo === "marca" && !cargando && busqueda.length >= 2 && items.length === 0 && !mostrarInputNuevo && (
          ...
          {tipo === "modelo" && !cargando && items.length === 0 && !esMarcaNueva && !esPrimeraCarga && !mostrarInputNuevo && (
            <div className="text-center py-4 border-t border-gray-200">
              <p className="text-gray-500">
          ...
          {/* Mensaje especial para marcas nuevas - SOLO si no está en modo creación */}
          {tipo === "modelo" && esMarcaNueva && !mostrarInputNuevo && (
            <div className="text-center py-4 border-t border-gray-200">
              <p className="text-gray-500">
        ...
        {/* Footer - Solo mostrar si no está en modo creación */}
        {!mostrarInputNuevo && (
