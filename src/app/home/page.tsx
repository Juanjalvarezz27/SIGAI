...
function puedeVerRuta(
  route: Route,
  userRol: ReturnType<typeof useUserRol>["userRol"]
): boolean {
  if (!userRol) {
    return false;
  }

  const userRole = userRol.rol;
  const userRolId = userRol.rolId;
  ...
  if (route.path === "/home/eventosExternos") {
    ...
    if (
      userRole === "admin" ||
      userRolId === 3 ||
      (userRolId === 2 && userRol.supervisorTipoId === 1)
    ) {
      return true;
    }
    return false;
...
function Inicio() {
  ...
  return (
    <>
      ...
      {/* Elimina z-[-1] del contenedor principal y úsalo solo donde sea necesario */}
      <div className="w-11/12 mx-auto">
        {/* Grid de botones estilo bento para desktop */}
        <div className="grid grid-cols-3 gap-6 w-10/12 mx-auto mt-8 relative">
          {availableRoutes.map((route, index) => {
            ...
            return (
              <Link
                ...
                href={route.path}
                className={`
                  group relative p-8 rounded-2xl border-2 border-gray-100 z-0
